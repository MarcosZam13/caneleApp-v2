// EditarPedidoSheet.tsx — Sheet para editar un pedido existente (wizard de 3 pasos)
'use client'

import { useState, useTransition, useMemo, useRef, useEffect } from 'react'
import { format } from 'date-fns'
import { Pencil, Trash2, Loader2, ArrowLeft, ArrowRight, Check, User, Package2, ClipboardList, Search, MapPin, X, PlusCircle } from 'lucide-react'
...
import { getPedidoById, updatePedido } from '@/actions/pedidos.actions'
...
interface EditarPedidoSheetProps {
  idPedido: string
  clientes: ClienteOpt[]
  rutas: RutaOpt[]
  iconOnly?: boolean
...
export function EditarPedidoSheet({ idPedido, clientes, rutas, iconOnly = false }: EditarPedidoSheetProps) {
  ...
  const clienteSeleccionado = useMemo(() => clientes.find(c => c.id_cliente === idCliente), [clientes, idCliente])
  const rutaSeleccionada = useMemo(() => rutas.find(r => r.id_ruta === idRuta), [rutas, idRuta])
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      ...
      if (rutaRef.current && !rutaRef.current.contains(e.target as Node)) setRutaOpen(false)
    }
    document.addEventListener('mousedown', handleClickOutside)
    return () => document.removeEventListener('mousedown', handleClickOutside)
  }, [])

  // Al abrir el sheet, cargar datos del pedido
  async function handleOpen() {
    ...
    setIdRuta(rutaId)
    setRutaSearch(ruta?.nombre ?? '')
    setIdDireccion(pedido.id_direccion ?? '')
    setFecha(pedido.fecha ?? format(new Date(), 'yyyy-MM-dd'))
    setNotas(pedido.notas ?? '')
    setPrioritaria(pedido.prioritaria ?? false)

    const [dirs, prods] = await Promise.all([
      getDireccionesPorCliente(clienteId),
      getProductosConPrecio(clienteId),
    ])

    setDirecciones(dirs)
    setProductos(prods)

    // Pre-poblar items con los productos del pedido
    setItems(
      pedido.pedido_producto.map((pp) => ({
        ...
        precio_unitario: Number(pp.precio_unitario ?? 0),
      }))
    )
  ...
  function handleProductoChange(localId: string, idProducto: string) {
    ...
    setItems(prev => prev.map(i =>
      i.localId === localId ? { ...i, id_producto: idProducto, precio_unitario: prod?.precio_efectivo ?? 0 } : i
    ))
  }
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
              ...
              {step === 1 && (
                <div className="space-y-5 animate-in fade-in slide-in-from-right-2">
                      ...
                      {clienteOpen && (
                        <div className="absolute z-50 mt-1 w-full rounded-md border border-border bg-popover shadow-lg max-h-48 overflow-auto">
                          {clientesFiltrados.length === 0 ? (
                            <p className="text-xs text-muted-foreground px-3 py-4 text-center">No se encontraron clientes</p>
                          ) : (
                            clientesFiltrados.slice(0, 50).map((c) => (
                              <button key={c.id_cliente} type="button"
                                className={cn('w-full text-left px-3 py-2 text-sm hover:bg-accent transition-colors', c.id_cliente === idCliente && 'bg-accent font-medium')}
                      ...
                      {rutaOpen && (
                        <div className="absolute z-50 mt-1 w-full rounded-md border border-border bg-popover shadow-lg max-h-48 overflow-auto">
                          {rutasFiltradas.length === 0 ? (
                            <p className="text-xs text-muted-foreground px-3 py-4 text-center">No se encontraron rutas</p>
                          ) : (
                            rutasFiltradas.slice(0, 50).map((r) => (
                              <button key={r.id_ruta} type="button"
                                className={cn('w-full text-left px-3 py-2 text-sm hover:bg-accent transition-colors flex items-center justify-between', r.id_ruta === idRuta && 'bg-accent font-medium')}
                  ...
                  {idCliente && direcciones.length > 0 && (
                    <div className="space-y-1.5">
                      <Label className="text-xs">Dirección de entrega (opcional)</Label>
                      <Select value={idDireccion || '__none__'} onValueChange={v => setIdDireccion(v === '__none__' ? '' : v)}>
                        <SelectTrigger className="text-sm">
                          <SelectValue>
                            {idDireccion ? (direcciones.find(d => d.id_direccion === idDireccion)?.direccion_texto ?? 'Dirección seleccionada') : 'Sin dirección específica'}
                          </SelectValue>
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  {idCliente && productos.length > 0 && (
                    <div className="rounded-md bg-emerald-50 border border-emerald-200 px-3 py-2 text-xs text-emerald-700 flex items-center gap-2">
                      ...
                      {productos.length} productos disponibles para {clienteSeleccionado?.nombre}
                    </div>
                  )}
                </div>
              )}
              ...
              {step === 2 && (
                <div className="space-y-3 animate-in fade-in slide-in-from-right-2">
                  ...
                  {productos.length === 0 ? (
                    <p className="text-xs text-muted-foreground text-center py-8">Selecciona un cliente primero.</p>
                  ) : (
                    <>
                      {items.map((item, idx) => (
                        <div key={item.localId} className="rounded-lg border p-3 space-y-3 bg-muted/30">
                          <div className="flex items-center justify-between">
                            ...
                            {items.length > 1 && (
                              <Button type="button" variant="ghost" size="icon" className="h-6 w-6 text-destructive hover:text-destructive" onClick={() => removeItem(item.localId)}>
                                <Trash2 className="h-3.5 w-3.5" />
                              </Button>
                            )}
                          </div>

                          <Select value={item.id_producto || '__none__'} onValueChange={id => { if (id && id !== '__none__') handleProductoChange(item.localId, id) }}>
                            <SelectTrigger className="text-sm">
                              <SelectValue placeholder="Selecciona un producto...">
                                {item.id_producto ? (productos.find(p => p.id_producto === item.id_producto)?.nombre ?? 'Producto seleccionado') : 'Selecciona un producto...'}
                              </SelectValue>
                            </SelectTrigger>
                              ...
                              {productos.map(p => (
                                <SelectItem key={p.id_producto} value={p.id_producto}>
                                  <div className="flex items-center justify-between gap-3 w-full pr-1">
                                    ...
                                    <span className="text-xs text-muted-foreground shrink-0 ml-auto">₡{p.precio_efectivo.toLocaleString('es-CR')}</span>
                                  </div>
                                </SelectItem>
                              ))}
                            </SelectContent>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1">
                              ...
                              <Input type="number" min={1} className="h-8 text-sm" value={item.cantidad}
                                onChange={e => updateItem(item.localId, 'cantidad', Math.max(1, parseInt(e.target.value) || 1))} />
                            </div>
                            <div className="space-y-1">
                              <Label className="text-xs">Precio unitario (₡)</Label>
                              <Input type="number" min={0} className="h-8 text-sm" value={item.precio_unitario}
                                onChange={e => updateItem(item.localId, 'precio_unitario', parseFloat(e.target.value) || 0)} />
                            </div>
                          </div>
              ...
              {step === 3 && (
                <div className="space-y-5 animate-in fade-in slide-in-from-right-2">
                  ...
                  <div className="rounded-lg border p-4 space-y-3 bg-muted/20">
                    ...
                    {idDireccion && (
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">{direcciones.find(d => d.id_direccion === idDireccion)?.direccion_texto ?? 'Seleccionada'}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-sm">
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="fecha-edit" className="text-xs">Fecha *</Label>
                    <Input id="fecha-edit" type="date" className="text-sm" value={fecha} onChange={e => setFecha(e.target.value)} required />
                  </div>

                  <div className="space-y-1.5">
                    <Label htmlFor="notas-edit" className="text-xs">Notas</Label>
                    <textarea id="notas-edit" className="flex min-h-[72px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 resize-none" placeholder="Instrucciones especiales..." value={notas} onChange={e => setNotas(e.target.value)} />
                  </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                {step < 3 ? (
                  ...
                  <Button type="submit" className="gap-2 ml-auto text-sm" disabled={isPending}>
                    ...
                    {isPending ? 'Guardando...' : `Guardar cambios${total > 0 ? ` · ₡${total.toLocaleString('es-CR')}` : ''}`}
                  </Button>
                )}
              </div>
            </form>
